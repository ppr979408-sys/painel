...
// MySQL Storage - Real database implementation with fallback
export class MySQLStorage implements IStorage {
  private connection: mysql.Connection | null = null;
  private connectionFailed = false;
  private fallbackStorage = new TestStorage();

  private async getConnection(): Promise<mysql.Connection> {
    if (this.connectionFailed) {
      throw new Error('MySQL connection previously failed, using fallback');
    }
    ...
    if (!this.connection) {
      try {
        ...
        console.log('✅ Conectado ao MySQL do InfinityFree com sucesso!');
        
      } catch (error) {
        this.connectionFailed = true;
        this.connection = null;
        console.log('⚠️  Falha na conexão MySQL, usando dados simulados baseados no schema real');
        throw error;
      }
    }
  ...
  private async executeWithFallback<T>(
    operation: () => Promise<T>,
    fallbackMethod: keyof TestStorage
  ): Promise<T> {
    try {
      return await operation();
    } catch (error) {
      console.log(`⚠️  MySQL indisponível, usando dados simulados para ${fallbackMethod}`);
      return (this.fallbackStorage[fallbackMethod] as any)(...Array.from(arguments).slice(2));
    }
  }
